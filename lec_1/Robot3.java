...
public class Robot3 {
    ...
    public void power() { 
        if (this.state == State.Off) { //если выключен
            this.powerOn(); //включаем
            this.state = State.On; //включен
        } else {
            this.powerOff(); //если включен выключаем
            this.state = State.Off; //состоянние выключен
        }
        System.out.println();
    }

    private void powerOn() { //включен
        this.startBIOS();
        this.startOS();
        this.sayHi();
    }

    private void powerOff() { //выключен
        this.sayBye();
        this.stopOS();
        this.stopBIOS();
    }
